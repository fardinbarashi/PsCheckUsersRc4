...
Title : RC4 Checker
Description : This Script checks if a enabled user in the ad-group got a RC4 encryption
Version : 1.0
Release day : 
...
#----------------------------------- Start Script ------------------------------------------
# Section 1 : Connect to $Adgroup and process user that have RS4 enabled
$Section = "Section 1 : Connect to $AdGroupName and process user that have RS4 enabled"
Try
 { # Start Try, $Section
  ...
  # Connect to AD-Group 
    $AdGroupName = ""
    $CheckAdGroup = Get-ADGroupMember -Identity $AdGroupName

    # Process users in $CheckAdGroup, Check if user got RC4 Enabled or not
    $Users = Foreach ($UserAccount in $CheckAdGroup) { Get-ADUser -Identity $UserAccount.SamAccountName -Properties *  }
